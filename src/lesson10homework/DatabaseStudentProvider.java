...
public class DatabaseStudentProvider implements StudentProvider {
	...
	protected Connection getConnection() throws SQLException {
		return DriverManager.getConnection("jdbc:postgresql://localhost:5432/myDBPostgres", "user", "root");
	}
	...
	public List<Student> findByAge(int age, int offset, int limit) {
		try (Connection c = getConnection()) {
			return JDBCUtils.select(c, "select * from student where age=? limit ? offset ?", LIST_STUDENTS_HANDLER, age,
					limit, offset);
		} catch (SQLException e) {
			throw new ProviderRetrieveException("Can't execute SQL query: " + e.getMessage(), e);
		}
	...
	public void create(Student student) {
		try (Connection c = getConnection()) {
			Student insertedStudent = JDBCUtils.insert(c, "insert into student values(nextval('student_id_seq'),?,?,?)",
	...
	public void deleteById(final Long id) {
		 ...
		 * 
		 * @Override public Void execute(Connection c) throws SQLException { int result
		 * = JDBCUtils.executeUpdate(c, "delete from student where id=?", id); if
		 * (result == 0) { throw new
		 * ProviderRetrieveException("Student not found by id="+id); } return null; }
		 * });
		 */
	}
