...
public class DefaultIOUtils implements IOUtils {
	...
	public void replaceChars(Reader in, Writer out, String inChars, String outChars)
		...
		if (inChars.length() != outChars.length()) {
			throw new IllegalArgumentException("inChars.length != outChars.length: inChars.length=" + inChars.length()
					+ ", outChars.length=" + outChars.length());
		}
	...
	@Override
	public List<Path> findText(Path rootDir, final String textToFind, final boolean ignoreCase)
			throws NullPointerException, IllegalArgumentException, IOException {
